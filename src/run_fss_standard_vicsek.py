"""
Sanity-check FSS with the standard Vicsek update (no Couzin zones,
no blind sector). The model used in the main text is a zonal
Vicsek-Couzin variant, so PRE referees will ask whether the heavy-tail
erosion of FSS is specific to the zonal extras. Standard Vicsek is
recovered with R_r = 0 (empty repulsion set, all visible neighbours
fall in the alignment branch) and beta = 0 (no rear blind sector).

Output: data/fss_standard.npz
"""
...
def main():
    ...
    R_a = 0.7  # same alignment radius as the main model for comparability
    ...
    pbar = tqdm(total=n_L * n_a * n_e * n_s, desc="standard")
            ...
            for ie, eta in enumerate(etas):
                for isd, sd in enumerate(seeds):
                    p = VicsekParams(
                        ...
                        R_r=0.0,        # no repulsion zone
                        R_a=R_a,
                        beta=0.0,       # no rear blind sector
    ...
    np.savez_compressed(
        DATA / "fss_standard.npz",
        Ls=Ls, Ns=Ns, alphas=alphas, etas=etas, seeds=seeds,
        phi=phi, chi=chi, U4=U4,
        sigma=np.array(sigma),
        params=np.array([v0, R_a, n_warm, n_meas], dtype=float),
    )
    print("saved:", DATA / "fss_standard.npz")
