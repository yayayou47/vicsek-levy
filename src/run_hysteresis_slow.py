"""
Slower hysteresis test (T_up = T_down = 32000) to check whether the
"no loop at alpha = 1" finding is genuine or a ramp-speed artefact.

Output: data/hysteresis_slow.npz
"""
...
def main():
    ...
    np.savez_compressed(
        DATA / "hysteresis_slow.npz",
        alphas=alphas, eta_path=eta_path, direction=direction,
        phi_traj=phi_traj,
        params=np.array([N, L, v0, R_r, R_a, T_relax, T_up, T_down,
                         len(seeds)], dtype=float),
    )
    print("saved:", DATA / "hysteresis_slow.npz")
